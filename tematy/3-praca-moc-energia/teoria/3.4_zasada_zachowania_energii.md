...
**Uwaga:** energia mechaniczna $E_c$ jest skalarem — to suma dwóch skalarów ($E_k$ i $E_p$), więc dodajemy je zwyczajnie jak liczby, bez żadnych "wektorowych" trików typu metoda równoległoboku (zobacz temat 0.6).
...
2. Na początku (na wysokości $h$) cała energia mechaniczna to energia potencjalna: $E_c = E_p = m \cdot g \cdot h$.
3. Tuż przed ziemią ($h = 0$) cała energia mechaniczna to energia kinetyczna: $E_c = E_k = \frac{1}{2} m v^2$.
