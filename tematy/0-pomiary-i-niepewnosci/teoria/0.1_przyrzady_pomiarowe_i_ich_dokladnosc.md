...
- **Niepewność pomiaru (dokładność przyrządu)** — liczba mówiąca, o ile odczytany wynik może się różnić od wartości prawdziwej.
  - Dla przyrządów **analogowych** (ze skalą i wskazówką albo słupkiem, np. termometr cieczowy, siłomierz, taśma miernicza) przyjmujemy, że niepewność pomiaru to **cała działka elementarna** (np. linijka z kreskami co 1 mm → niepewność ±1 mm). Tak przyjmują polskie podręczniki szkolne (m.in. *Spotkania z fizyką*, materiały zpe.gov.pl) — i taką konwencję stosujemy w całym tym repozytorium oraz na konkursie, chyba że treść zadania mówi inaczej.
...
> **Uwaga o innej konwencji (rozszerzenie):** w niektórych źródłach — zwłaszcza zagranicznych i akademickich (laboratoria uczelniane) — spotyka się konwencję, że niepewność odczytu ze skali analogowej to *połowa* działki elementarnej (bo oko potrafi ocenić, do której kreski wskazaniu bliżej). Nie jest to błąd, tylko inna umowa. Jeśli zadanie samo podaje dokładność przyrządu — zawsze użyj wartości z treści zadania; jeśli nie podaje — przyjmij całą działkę elementarną.
...
>
> **Odpowiedź:** Nie — nawet teoretycznie. Każda skala pomiarowa ma jakąś najmniejszą działkę, a granicą ostateczną jest budowa materii (skala pomiarowa złożona jest z atomów, które mają swój rozmiar). Niepewność można zmniejszać — lepszym przyrządem, lepszą metodą — ale nigdy nie da się jej sprowadzić do idealnego zera.
...
| Termometr cieczowy | temperatura | np. 1°C lub 2°C | działka elementarna (np. ±1°C lub ±2°C) |
| Barometr | ciśnienie | zależnie od modelu | z klasy dokładności |
| Taśma miernicza | długość (duże odległości) | 1 mm lub 1 cm | z klasy dokładności lub działka elementarna |
| Siłomierz | siła | np. 0,1 N | działka elementarna (np. ±0,1 N) |
