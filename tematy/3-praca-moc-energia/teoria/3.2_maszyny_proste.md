...
$$F \cdot s = Q \cdot h \qquad \text{czyli} \qquad F = \frac{Q \cdot h}{s}$$

gdzie:

- $F$ — siła potrzebna do wciągnięcia ciała wzdłuż równi (wektor — tu użyta wartość)
- $Q$ — ciężar ciała (wektor — tu użyta wartość), $Q = m \cdot g$
...
**Dziwne pytanie:** Do wejścia na tę samą wysokość mogę użyć krótkich, stromych schodów albo długiej, łagodnej rampy dla wózków. Pomijając tarcie, praca potrzebna do wniesienia tego samego ciężaru na tę samą wysokość jest identyczna ($W = Q \cdot h$, niezależnie od trasy) — to skąd bierze się wrażenie, że rampa jest "łatwiejsza"?
...
1. Dane: $s = 5\ \text{m}$, $h = 1\ \text{m}$, $Q = 250\ \text{N}$.
2. Wzór: $F = \dfrac{Q \cdot h}{s}$.
