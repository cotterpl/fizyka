...
Wykresy to "język", którym fizycy opisują ruch bez słów. Warto wiedzieć, jak wyglądają dla trzech podstawowych rodzajów ruchu.
...
- Krzywa **(II)** pokazuje wariant pośredni — ciało zwalnia z tym samym opóźnieniem, ale tylko do pewnej mniejszej prędkości $v_1$ (nie do zera), a potem jedzie dalej ze stałą prędkością $v_1$ — stąd załamanie na wykresie s(t) z krzywej w prostą.
- Krzywa **(III)** to ruch jednostajny bez żadnego hamowania — dodana tylko dla porównania (linia pozioma na wykresie v(t), na stałej wysokości $v_0$, i linia prosta ukośna na wykresie s(t)).
...
### Dziwne pytanie: czy wykres drogi s(t) może "iść w dół"?

Wróć do przykładu z rozdziału 1.1 — Marek na deskorolce jedzie od ławki do latarni (`5 m`), a potem zawraca do ławki. Jeśli narysujemy wykres **drogi** s(t) dla całej tej wycieczki, to — mimo że Marek w drugiej części wraca do punktu startu — wykres s(t) **nigdy nie może się obniżać**! Droga to suma wszystkich przebytych odcinków, więc może tylko rosnąć albo (podczas postoju) być pozioma — nigdy nie maleje, nawet gdy ciało realnie zawraca i wraca do punktu wyjścia.

Wykres, który by się obniżał, opisywałby inną wielkość — *położenie* (informację, w którym miejscu na trasie akurat jesteśmy), a nie drogę. To rozróżnienie (droga a przemieszczenie) omawiamy szczegółowo, wraz z przykładem dla ruchu przyspieszonego, w rozszerzeniu 1.7. W tym rozdziale zajmujemy się wyłącznie drogą, więc pamiętaj: **wykres s(t) jest zawsze niemalejący**, niezależnie od tego, jak skomplikowaną trasę (nawet z zawracaniem) pokonuje ciało.
...
2. Fragment I (`0–3 s`, trójkąt): $s_1 = \frac12 \cdot 3\,\text{s} \cdot 6\,\text{m/s} = 9\ \text{m}$.
3. Fragment II (`3–8 s`, prostokąt, bo prędkość stała): $s_2 = 6\,\text{m/s} \cdot 5\,\text{s} = 30\ \text{m}$.
4. Droga całkowita: $s = s_1 + s_2 = 9\,\text{m} + 30\,\text{m} = 39\ \text{m}$.
