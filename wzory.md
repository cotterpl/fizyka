...
gdzie: $m$ — masa ciała (skalar), $\vec{g}$ — przyspieszenie ziemskie (wektor; w przybliżeniu $9{,}81\ \text{m/s}^2$, w zadaniach często zaokrąglane do $10\ \text{m/s}^2$; przy obliczaniu ciężaru i w hydrostatyce podręczniki SP zapisują tę samą wartość w jednostce N/kg, bo $1\ \text{N/kg} = 1\ \text{m/s}^2$), $\vec{F_g}$ — ciężar (wektor).
...
**Złota reguła mechaniki** (praca włożona = praca uzyskana w idealnej maszynie prostej, bez tarcia — dotyczy bloków, kołowrotu i równi pochyłej, które są zastosowaniami tej reguły; wzór na wartościach, bez strzałek):
...
**Warunek równowagi dźwigni** (zasada momentów sił — dotyczy też kołowrotu, który jest dźwignią "nawiniętą" na oś; wzór na wartościach, bez strzałek):
...
**Napięcie powierzchniowe** (*rozszerzenie — w szkole podstawowej zjawisko opisuje się tylko jakościowo, wzór wykracza poza podstawę programową*; $l$ — długość linii brzegowej powierzchni cieczy, skalar):
...
gdzie: $F_w$ — siła wyporu (wektor — tu użyta wartość), $\rho_{\text{cieczy}}$ — gęstość cieczy, w której zanurzone jest ciało (skalar), $V_{\text{zanurzone}}$ — objętość zanurzonej części ciała (skalar), $g$ — przyspieszenie ziemskie (wektor — tu użyta wartość). Prawo Archimedesa dotyczy też gazów — wtedy zamiast $\rho_{\text{cieczy}}$ wstawiamy gęstość gazu (np. powietrza).
