...
Jednostkę zapisujemy zawsze po całym wyrażeniu, np. $l = (143{,}5 \pm 0{,}1)\ \text{mm}$ — nigdy osobno przy każdej liczbie.

![Oś liczbowa z zaznaczonym przedziałem niepewności pomiaru długości ołówka: l = (143,5 ± 0,1) mm](ilustracje/os-niepewnosc-dlugosc-olowka.svg)

*Ilustracja własna (nie znaleziono gotowego, licencjonowanego obrazka pasującego dokładnie do tego przykładu liczbowego 143,5 mm ± 0,1 mm).*

Wartość PRAWDZIWA długości ołówka na pewno leży gdzieś w przedziale od `143,4 mm` do `143,6 mm` — po prostu nie wiemy, w którym dokładnie miejscu tego przedziału.
...
### Niepewność wielkości wyznaczonej pośrednio (przez dodawanie/odejmowanie)

Czasem szukana wielkość nie jest mierzona bezpośrednio, tylko obliczana z dwóch (lub więcej) osobnych pomiarów — np. masę wody w szklance wyznaczamy, odejmując masę pustej szklanki od masy szklanki z wodą. W takim przypadku niepewności pomiarów **zawsze się dodają**, niezależnie od tego, czy same wielkości dodajemy, czy odejmujemy — bo w najgorszym razie błędy obu pomiarów mogą się nałożyć w tym samym kierunku:

$$\Delta(x - y) = \Delta(x + y) = \Delta x + \Delta y$$
...
### Przykład 2 (niepewność przy odejmowaniu)

**Treść:** Aby wyznaczyć masę wody w szklance, zważono osobno szklankę pustą: m = (130 ± 5) g, a potem szklankę z wodą: M = (385 ± 5) g. Oblicz masę wody wraz z niepewnością.

**Rozwiązanie:**

1. Masa wody to różnica obu pomiarów: $w = M - m = 385\ \text{g} - 130\ \text{g} = 255\ \text{g}$.
2. Niepewności się dodają: $\Delta w = \Delta M + \Delta m = 5\ \text{g} + 5\ \text{g} = 10\ \text{g}$.

**Odpowiedź:** $w = (255 \pm 10)\ \text{g}$. Prawdziwa masa wody mieści się między 245 g a 265 g.
